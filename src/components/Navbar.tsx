  ...
  return (
    <>
      ...
      >
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="hidden lg:flex items-center space-x-10">
              {navLinks.map((link) => (
                <a
                  key={link.name}
                  href={link.href}
                  className="text-[11px] font-bold uppercase tracking-[0.15em] text-slate-500 hover:text-primary-600 transition-colors"
                >
                  {link.name}
                </a>
              ))}
            ...
            <div className="hidden lg:flex items-center space-x-6">
              ...
              <a
                ...
                className="text-slate-900 font-bold text-xs flex items-center hover:text-primary-600 transition-colors"
              >
                <Phone className="w-3 h-3 mr-2 text-primary-600" />
                <span>+49 176 22006681</span>
              </a>

              <a
                href="#contact"
                className="bg-slate-900 text-white px-5 py-2.5 rounded-lg font-bold text-[11px] uppercase tracking-widest flex items-center hover:bg-primary-600 transition-all active:scale-95"
              >
        ...
        <div
          className={`lg:hidden fixed inset-x-0 top-[60px] bg-white border-b border-zinc-100 transition-all duration-300 transform origin-top ${isMenuOpen ? 'scale-y-100 opacity-100' : 'scale-y-0 opacity-0 pointer-events-none'
            }`}
        >
          <div className="px-8 py-10 space-y-6">
            ...
            <div className="pt-6 border-t border-zinc-100">
              <a
                href="#contact"
                onClick={() => setIsMenuOpen(false)}
                className="block bg-primary-600 text-white text-center py-4 rounded-xl font-bold text-lg"
              >
                {t('nav.freeQuote')}
              </a>
            </div>
